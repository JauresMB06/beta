...
class _DoctorDashboardState extends State<DoctorDashboard> {
  ...
  void openPatientDetail(int id, String name) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PatientDetailScreen(token: token!, patientId: id, patientName: name),
      ),
    );
  }
      ...
      body: loading
          ...
          : ListView(
              children: [
                ...
                Divider(),
                ListTile(
                  title: Text("Chatbot History", style: TextStyle(fontWeight: FontWeight.bold)),
                ),
                ...chatlogs.map((c) => ListTile(
                      title: Text("Q: ${c["message"]}"),
                      subtitle: Text("A: ${c["response"]}\nAt: ${c["timestamp"]}"),
                    )),
              ],
            ),
    );
